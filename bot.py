...
# ── Weighted spicy tag pools ───────────────────────────────────────────────────
# Each entry is (tag, weight). Higher weight = picked more often.
# Used by providers that support tag filtering (rule34, gelbooru, danbooru, etc.)

WEIGHTED_TAGS = [
    # ── MILF / mature (heavily weighted per preference) ──────────────────
    ("milf",                    18),
    ("mature_female",           16),
    ("older_woman",             10),
    ("mother",                   8),
    ("big_breasts milf",        14),
    ("large_breasts milf",      12),
    ("milf paizuri",            10),
    ("milf blowjob",            10),
    ("milf sex",                 9),
    ("milf riding",              9),
...
# Build flat list + weights for random.choices()
_TAG_NAMES   = [t for t, _ in WEIGHTED_TAGS]
_TAG_WEIGHTS = [w for _, w in WEIGHTED_TAGS]

def _pick_tag() -> str:
    """Pick a random spicy tag using weighted selection."""
    return random.choices(_TAG_NAMES, weights=_TAG_WEIGHTS, k=1)[0]

def _pick_tags(n: int = 1) -> list:
    """Pick n unique spicy tags."""
    chosen, pool = [], list(range(len(_TAG_NAMES)))
    random.shuffle(pool)
    seen = set()
    for i in pool:
        base = _TAG_NAMES[i].split()[0]  # avoid duplicate base tags
        if base not in seen:
            chosen.append(_TAG_NAMES[i])
            seen.add(base)
        if len(chosen) >= n:
            break
    return chosen

# Legacy flat lists kept for any code that references them
SPICY_TAGS = _TAG_NAMES
GIF_TAGS = [
...
async def fetch_rule34(session, positive=None):
    """Rule34 — tag-weighted 70% of the time, pure explicit 30%."""
    try:
        if random.random() < 0.70:
            tag  = _pick_tag()
            tags = f"rating:explicit {tag}"
        else:
            tags = "rating:explicit"
        params = {
            "page": "dapi", "s": "post", "q": "index",
            "json": "1", "tags": tags, "limit": 200,
        }
        to = aiohttp.ClientTimeout(total=REQUEST_TIMEOUT)
        async with session.get("https://api.rule34.xxx/index.php", params=params, timeout=to) as resp:
            ...
            if not posts:
                # Fallback: retry without tag
                params["tags"] = "rating:explicit"
                async with session.get("https://api.rule34.xxx/index.php", params=params, timeout=to) as resp2:
                    if resp2.status != 200: return None, None, None
...
async def fetch_gelbooru(session, positive=None):
    """Gelbooru — tag-weighted 70% of the time."""
    tag = _pick_tag() if random.random() < 0.70 else None
    extra = [tag] if tag else None
    url, _, post = await _gelbooru_compat(
        session, "https://gelbooru.com/index.php",
        GELBOORU_API_KEY or None, GELBOORU_USER or None,
        extra_tags=extra
    )
    if url is None and extra:
...
async def fetch_konachan(session, positive=None):
    """Konachan — tag-weighted 65% of the time."""
    try:
        ...
        async with session.get("https://konachan.com/post.json", params=params, headers=hdrs, timeout=to) as resp:
            ...
            if not posts:
                params["tags"] = "rating:explicit"
                async with session.get("https://konachan.com/post.json", params=params, headers=hdrs, timeout=to) as resp2:
                    if resp2.status != 200: return None, None, None
                    posts = await resp2.json(content_type=None)
                    if not posts: return None, None, None
            image_posts = [
...
async def fetch_nekobot(session, positive=None):
    """Nekobot — expanded category list including milf."""
    try:
        category = random.choice([
            ...
            # Extra weight on milf by listing more times
            "hmilf", "hmilf", "hboobs", "paizuri",
        ])
        to = aiohttp.ClientTimeout(total=REQUEST_TIMEOUT)
...
async def fetch_danbooru(session, positive=None):
    """Danbooru — tag-weighted 65% of the time."""
    try:
        if random.random() < 0.65:
            tag   = _pick_tag()
            tags  = f"rating:explicit {tag}"
            ...
            tags  = "rating:explicit"
        params = {"tags": tags, "limit": 100, "random": "true"}
        ...
        async with session.get("https://danbooru.donmai.us/posts.json", params=params, auth=auth, timeout=to) as resp:
            ...
            if not posts:
                params["tags"] = "rating:explicit"
                async with session.get("https://danbooru.donmai.us/posts.json", params=params, auth=auth, timeout=to) as resp2:
                    if resp2.status != 200: return None, None, None
                    posts = await resp2.json(content_type=None)
                    if not posts: return None, None, None
            post = random.choice(posts)
...
async def fetch_e621(session, positive=None):
    """e621 — tag-weighted 65% of the time."""
    try:
        if random.random() < 0.65:
            tag  = _pick_tag()
            tags = f"rating:explicit order:random {tag}"
        else:
        ...
        async with session.get("https://e621.net/posts.json", params=params, headers=hdrs, auth=auth, timeout=to) as resp:
            ...
            if not posts:
                params["tags"] = "rating:explicit order:random"
                async with session.get("https://e621.net/posts.json", params=params, headers=hdrs, auth=auth, timeout=to) as resp2:
                    if resp2.status != 200: return None, None, None
                    posts = (await resp2.json(content_type=None)).get("posts", [])
                    if not posts: return None, None, None
            image_posts = [
                p for p in posts
...
async def fetch_yandere(session, positive=None):
    """Yande.re — tag-weighted 65% of the time."""
    try:
        if random.random() < 0.65:
            tag  = _pick_tag()
            tags = f"rating:explicit order:random {tag}"
        ...
        async with session.get("https://yande.re/post.json", params=params, headers=hdrs, timeout=to) as resp:
            ...
            if not posts:
                params["tags"] = "rating:explicit order:random"
                async with session.get("https://yande.re/post.json", params=params, headers=hdrs, timeout=to) as resp2:
                    if resp2.status != 200: return None, None, None
                    posts = await resp2.json(content_type=None)
                    if not posts: return None, None, None
            image_posts = [
...
async def fetch_hmtai(session, positive=None):
    """hmtai — expanded with extra weight on boobs/paizuri/creampie."""
    try:
        category = random.choice([
            ...
            # extra weight
            "boobs", "paizuri", "hboobs", "creampie", "blowjob", "ass",
        ])
        to = aiohttp.ClientTimeout(total=REQUEST_TIMEOUT)
        async with session.get(
...
# ── Dedicated spicy/milf providers ────────────────────────────────────────────

async def fetch_rule34_milf(session, positive=None):
    """Rule34 — milf-focused tag pool, always uses a milf/mature tag."""
    try:
        tag = random.choices(
            ["milf", "mature_female", "big_breasts milf", "milf paizuri",
             "milf blowjob", "milf riding", "milf sex", "milf stockings",
             "milf cum", "older_woman", "milf anal", "milf gangbang",
             "milf creampie", "milf femdom", "mother"],
            weights=[20, 14, 13, 12, 12, 11, 11, 9, 9, 8, 8, 7, 7, 6, 5],
            k=1
        )[0]
        params = {
            "page": "dapi", "s": "post", "q": "index",
            "json": "1", "tags": f"rating:explicit {tag}", "limit": 200,
        }
        to = aiohttp.ClientTimeout(total=REQUEST_TIMEOUT)
        async with session.get("https://api.rule34.xxx/index.php", params=params, timeout=to) as resp:
            if resp.status != 200: return None, None, None
            ...
            if not gif_url or gif_url.lower().endswith((".webm", ".mp4", ".swf")):
                return None, None, None
            return gif_url, "rule34_milf", post
    except Exception:
        return None, None, None

async def fetch_gelbooru_milf(session, positive=None):
    """Gelbooru — milf-focused tag pool."""
    tag = random.choices(
        ["milf", "mature_female", "big_breasts milf", "milf paizuri",
         "milf blowjob", "milf riding", "milf sex", "older_woman",
         "milf creampie", "milf stockings", "milf lingerie"],
        weights=[20, 14, 13, 12, 12, 11, 11, 8, 8, 7, 6],
        k=1
    )[0]
    url, _, post = await _gelbooru_compat(
        session, "https://gelbooru.com/index.php",
    ...
    return url, "gelbooru_milf", post

async def fetch_danbooru_milf(session, positive=None):
    """Danbooru — milf-focused."""
    try:
        tag = random.choices(
            ["milf", "mature_female", "big_breasts milf", "milf paizuri",
             "milf blowjob", "milf sex", "older_woman", "milf creampie"],
            weights=[20, 14, 13, 12, 12, 11, 8, 8],
            k=1
        )[0]
        params = {"tags": f"rating:explicit {tag}", "limit": 100, "random": "true"}
        auth = aiohttp.BasicAuth(DANBOORU_USER, DANBOORU_API_KEY) if (DANBOORU_USER and DANBOORU_API_KEY) else None
        to = aiohttp.ClientTimeout(total=REQUEST_TIMEOUT)
        async with session.get("https://danbooru.donmai.us/posts.json", params=params, auth=auth, timeout=to) as resp:
            if resp.status != 200: return None, None, None
            ...
            if not gif_url or gif_url.lower().endswith((".webm", ".mp4", ".swf")):
                return None, None, None
            return gif_url, "danbooru_milf", post
    except Exception:
        return None, None, None

async def fetch_rule34_paizuri(session, positive=None):
    """Rule34 — paizuri/big breasts focused."""
    try:
        tag = random.choices(
            ["paizuri", "big_breasts", "large_breasts", "huge_breasts",
             "breast_fuck", "oppai", "breast_grab", "nipples", "lactation",
             "paizuri milf", "big_breasts blowjob", "paizuri cum"],
            weights=[18, 16, 15, 13, 12, 11, 10, 9, 7, 10, 9, 8],
            k=1
        )[0]
        params = {
            "page": "dapi", "s": "post", "q": "index",
        ...
        async with session.get("https://api.rule34.xxx/index.php", params=params, timeout=to) as resp:
            ...
            if not gif_url or gif_url.lower().endswith((".webm", ".mp4", ".swf")):
                return None, None, None
            return gif_url, "rule34_paizuri", post
    except Exception:
        return None, None, None

async def fetch_xbooru_spicy(session, positive=None):
    """Xbooru — tag-injected for spicy content."""
    tag = _pick_tag()
    url, _, post = await _gelbooru_compat(session, "https://xbooru.com/index.php", extra_tags=[tag])
    if url is None:
        url, _, post = await _gelbooru_compat(session, "https://xbooru.com/index.php")
    return url, "xbooru_spicy", post

async def fetch_tbib_spicy(session, positive=None):
    """TBIB — tag-injected for spicy content."""
    tag = _pick_tag()
    url, _, post = await _gelbooru_compat(session, "https://tbib.org/index.php", extra_tags=[tag])
    if url is None:
        url, _, post = await _gelbooru_compat(session, "https://tbib.org/index.php")
    return url, "tbib_spicy", post
...
_BASE_PROVIDERS = [
    ...
    ("gelbooru_popular",   fetch_gelbooru_popular,     14),
    ...
    # ── Spicy/tag-injected ────────────────────────────────────────────────
    ("xbooru_spicy",       fetch_xbooru_spicy,         10),
    ("tbib_spicy",         fetch_tbib_spicy,            9),
    ("rule34_paizuri",     fetch_rule34_paizuri,       12),  # boobs/paizuri focus
    # ── Standard pools ───────────────────────────────────────────────────
    ("tbib",               fetch_tbib,                  5),
    ("xbooru",             fetch_xbooru,                5),
    ...
    ("paheal_popular",     fetch_rule34_paheal_popular, 4),
    ...
    ("behoimi",            fetch_behoimi,               4),
]

_NSFW_EXTRA_PROVIDERS = [
    # ── MILF dedicated (high weight) ─────────────────────────────────────
    ("rule34_milf",        fetch_rule34_milf,          22),  # milf focus
    ("gelbooru_milf",      fetch_gelbooru_milf,        18),  # milf focus
    ("danbooru_milf",      fetch_danbooru_milf,        12),  # milf focus
    # ── General explicit ─────────────────────────────────────────────────
    ("e621",               fetch_e621,                 18),
